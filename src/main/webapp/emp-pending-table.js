// Get table element
let table5 = document.getElementById('pending-tickets-table');
...
const fetchPendingTicketsByUsername = () => {
	let hostname = window.location.hostname;

	console.log('fetchTicketsByUsername triggered');

	fetch(`http://${hostname}:8080/proj-01-team07/pending_tickets`)
		.then((response) => response.json())
		.then((data) => buildTable5(data));
};
